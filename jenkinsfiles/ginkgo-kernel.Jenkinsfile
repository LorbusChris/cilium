    ...
    stages {
        ...
        stage('Set programmatic env vars') {
            steps {
                ...
                script {
                    env.K8S_VERSION = sh script: '''#!/bin/bash
                        set -x
                        COMMENT_BODY_2=${COMMENT_BODY/\\'/}
                        flags1=(${ghprbCommentBody})
                        flags2=(${COMMENT_BODY})
                        flags3=(${COMMENT_BODY_2})
                        if [ "${ghprbCommentBody}" != "" ]; then
                            python ${TESTDIR}/get-gh-comment-info.py ${flags1[@]} --retrieve="k8s_version"
                            python ${TESTDIR}/get-gh-comment-info.py ${flags2[@]} --retrieve="k8s_version"
                            python ${TESTDIR}/get-gh-comment-info.py ${COMMENT_BODY/\\'/} --retrieve="k8s_version"
                            python ${TESTDIR}/get-gh-comment-info.py ${flags3[@]} --retrieve="k8s_version" | \
